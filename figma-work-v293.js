/* V293: pause preview video decode/playback only while Hero/Works scroll handoff is actively moving. */
(() => {
  ...
  /* V273 — one predictable primary-open route for thumbnail/title clicks.
     Workly + Serene go straight to Figma; the remaining three use the
     portfolio viewer so the interaction matches what each project actually has. */
  function primaryOpenAction(p){
    if (!p) return null;

    if (p.id === 'workly' || p.id === 'serene') {
      return (p.actions || []).find(action => action.kind === 'figma' && action.type === 'external')
        || (p.actions || []).find(action => action.type === 'external')
        || p.actions?.[0]
        || null;
    }

    return (p.actions || []).find(action =>
      action.kind === 'view' && ['pdf','image','video'].includes(action.type)
    ) || (p.actions || []).find(action => ['pdf','image','video'].includes(action.type))
      || p.actions?.[0]
      || null;
  }

  function openPrimaryProject(p){
    const action = primaryOpenAction(p);
    if (action) openProject(p, action);
  }
  ...
  let workInView = workSection
    ...
    : true;
  let heroTransitionActive = document.querySelector('.figma-hero-section')?.classList.contains('v242-hero-transitioning') || false;
  let workEntryActive = workSection?.classList.contains('v293-work-scroll-entering') || false;
  let workMediaResumeTimer = 0;
  ...
  function renderList(){
      ...
      <button class="figma-work-item ${p.id===highlightId?'is-active':''}" type="button" data-project="${p.id}" aria-pressed="${p.id===highlightId}">
        ...
        <span class="figma-work-main">
          <span class="figma-work-item-copy">
            <span class="figma-work-item-title" data-work-title-open>${p.listTitle}</span>
    ...
    list.querySelectorAll('.figma-work-item').forEach(item => {
      ...
      item.addEventListener('click',event=>{
        ...
        if (event.target.closest('[data-work-title-open]')) {
          event.preventDefault();
          event.stopPropagation();
          openPrimaryProject(project);
          return;
        }
        activate(id);
      });
  ...
  function renderStack(){
    ...
    preview.querySelectorAll('.figma-work-card').forEach(card => {
      ...
      const media = card.querySelector('.figma-work-media-shell');
      if (media) {
        media.setAttribute('role','link');
        media.setAttribute('data-v276-interactive','true');
        media.setAttribute('aria-label',`Open ${p.title}`);
        media.tabIndex = -1;
        media.addEventListener('click',event=>{
          if (!card.classList.contains('is-front')) return;
          event.preventDefault();
          openPrimaryProject(p);
        });
      }
    });
  ...
  function syncCardAccessibility(){
    stackOrder.forEach((id,index) => {
      ...
      card.querySelectorAll('video.figma-work-video-preview').forEach(video=>{
        ...
        if (front && workInView && !heroTransitionActive && !workEntryActive && document.visibilityState === 'visible' && !worklyReducedMotion) {
          video.play().catch(()=>{});
        } else {
          video.pause();
        }
  ...
  function resetWorkToLeadProject(){
    /* V276 — reset only on genuine viewport entry. Invalidate any async phase
       commit first, clear hover activation, then restore every inline stack
       property before snapping Workly to slot 0. This prevents re-entry from
       inheriting half-committed transforms/z-index values. */
    clearTimeout(hoverTimer);
    stackSerial += 1;
    cancelStackAnimations();
    ...
    highlightId = 'workly';
    previewId = 'workly';
    stackOrder = projects.map(project => project.id);
    ...
    /* Re-commit after layout has measured the visible section. This second
       clean snap is intentionally non-animated and prevents stale ResizeObserver
       measurements from making the stack look offset on re-entry. */
    requestAnimationFrame(() => {
      snapStack();
      syncStackScale();
    });
  ...
  if ('IntersectionObserver' in window && workSection) {
    let wasActuallyVisible = false;
    let firstWorkEntryPending = true;
    const workObserver = new IntersectionObserver(entries => {
      const entry = entries[0];
      const nextVisible = !!entry?.isIntersecting && entry.intersectionRatio >= 0.08;

      /* V277 — Workly is only the default for the visitor's first real entry
         into Selected Works. After that, leaving/re-entering preserves the
         last hovered/active project and its stack order, so no hidden Workly
         reset can overlap the current cards on re-entry. */
      if (nextVisible && !wasActuallyVisible && firstWorkEntryPending) {
        resetWorkToLeadProject();
        firstWorkEntryPending = false;
      }

      workInView = nextVisible;
      wasActuallyVisible = nextVisible;
      syncCardAccessibility();
    }, {
      root:null,
      rootMargin:'0px',
  ...
  const resyncTransitionMedia=()=>{
    clearTimeout(workMediaResumeTimer);
    if(heroTransitionActive || workEntryActive){
      preview.querySelectorAll('video.figma-work-video-preview').forEach(video=>{ try{video.pause();}catch(_){} });
      return;
    }
    workMediaResumeTimer=window.setTimeout(syncCardAccessibility,80);
  };
  window.addEventListener('hero:scroll-collapse-state',event=>{
    heroTransitionActive=!!event.detail?.active;
    resyncTransitionMedia();
  });
  window.addEventListener('work:scroll-entry-state',event=>{
    workEntryActive=!!event.detail?.active;
    resyncTransitionMedia();
  });
