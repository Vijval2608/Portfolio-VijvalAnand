...
(() => {
  ...
  // Emergency fallback only. Once the real motion controller has announced
  // hero:computer-ready, these timers are permanently disarmed.
  window.setTimeout(() => {
    if (!motionSequenceStarted && !started) startPowerOn();
  }, 4200);
  window.setTimeout(() => {
    if (!motionSequenceStarted && !normalTypingStarted && !bootTimer) startBootText();
  }, 4850);
  window.setTimeout(() => {
    if (!motionSequenceStarted) startNormalTyping();
  }, 8000);
})();
