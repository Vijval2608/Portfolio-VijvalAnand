(() => {
  ...
  if (artboard && screenWrap) {
    ...
    /* V282 — reward verification is deliberately separate from the visible
       score. The game remains fully client-rendered, but a genuine player win
       can receive a short server-signed proof after a lightweight physics
       trace is validated by the Netlify function. Nothing in this block
       changes paddle/ball physics or input timing. */
    const VERIFY_URL = '/api/pong-verify';
    const VERIFY_SAMPLE_MS = 80;
    let verifySessionPromise = null;
    let verifySession = '';
    let telemetry = null;
    let telemetryStart = 0;
    let lastTelemetrySample = -Infinity;
    let verifiedProof = '';
    let verifyPending = false;

    const compactNumber = n => Math.round(n * 100) / 100;

    const requestVerifySession = () => {
      verifiedProof = '';
      verifyPending = false;
      verifySession = '';
      telemetry = null;
      telemetryStart = 0;
      lastTelemetrySample = -Infinity;
      if (typeof fetch !== 'function') { verifySessionPromise = Promise.resolve(''); return; }
      verifySessionPromise = fetch(`${VERIFY_URL}?start=1`, {
        method:'GET',
        ...
        .then(data => {
          verifySession = data?.ok && typeof data.session === 'string' ? data.session : '';
          return verifySession;
        })
        .catch(() => '');
    };

    const beginTelemetry = () => {
      if (telemetry) return;
      if (!verifySessionPromise) requestVerifySession();
      telemetryStart = performance.now();
      lastTelemetrySample = -Infinity;
      telemetry = { frames:[], points:[] };
    };

    const telemetryTime = () => telemetryStart ? Math.max(0, Math.round(performance.now() - telemetryStart)) : 0;

    const captureTelemetryFrame = now => {
      if (!telemetry || !telemetryStart) return;
      const t = Math.max(0, Math.round(now - telemetryStart));
      if (t - lastTelemetrySample < VERIFY_SAMPLE_MS) return;
      lastTelemetrySample = t;
      if (telemetry.frames.length >= 9000) return;
      telemetry.frames.push({
        t,
        x:compactNumber(ball.x), y:compactNumber(ball.y),
        vx:compactNumber(ball.vx), vy:compactNumber(ball.vy),
        py:compactNumber(playerY), ay:compactNumber(aiY),
        ps:playerScore, as:aiScore, run:!!running
      });
    };

    const submitVerifiedWin = async () => {
      if (!telemetry || verifyPending || verifiedProof || playerScore !== 5 || aiScore >= 5) return;
      const submittedTelemetry = telemetry;
      const submittedFinal = { playerScore, aiScore };
      verifyPending = true;
      try {
        const session = verifySession || await verifySessionPromise || '';
        if (!session || telemetry !== submittedTelemetry) return;
        const payload = {
          version:1,
          session,
          frames:submittedTelemetry.frames,
          points:submittedTelemetry.points,
          final:submittedFinal
        };
        ...
        if (telemetry !== submittedTelemetry || !ended || playerScore !== submittedFinal.playerScore || aiScore !== submittedFinal.aiScore) return;
        if (data?.verified && typeof data.proof === 'string' && /^VJ-[A-Z0-9-]{16,80}$/.test(data.proof)) {
          verifiedProof = data.proof.slice(0,80);
          draw();
        }
      } catch (_) {
        /* Verification is intentionally non-blocking: gameplay never depends on network. */
      } finally {
        verifyPending = false;
      }
    };
    ...
    const draw = () => {
      ...
      text(String(playerScore),W/2-25,19,17,'center');
      text(String(aiScore),W/2+25,19,17,'center');
      text('PONG',W/2,19,9.6,'center',palette.dim);

      if (!running) {
        ctx.fillStyle = 'rgba(12,17,11,.82)';
        ctx.fillRect(18,64,W-36,78);
        if (ended) {
          text(winner, W/2, 83, 15.8, 'center', palette.accent);
          text('CLICK TO RESTART', W/2, 109, 10.5, 'center');
          if (verifiedProof) text(`VERIFIED · ${verifiedProof}`, W/2, 130, 6.6, 'center', palette.dim);
        } else {
          text('READY?', W/2, 81, 16.2, 'center', palette.accent);
          text('CLICK TO SERVE', W/2, 105, 11.4, 'center');
          text('MOUSE / W S', W/2, 127, 9.1, 'center', palette.dim);
        }
      }
      text('ESC EXIT',W-12,H-12,7.8,'right',palette.dim);
      ctx.restore();
    };
    ...
    const scorePoint = (playerWon) => {
      ...
      if (telemetry) {
        ...
        if (telemetry.frames.length < 9000) {
          telemetry.frames.push({
            t:pointT,
            x:compactNumber(ball.x), y:compactNumber(ball.y),
            vx:compactNumber(ball.vx), vy:compactNumber(ball.vy),
            py:compactNumber(playerY), ay:compactNumber(aiY),
            ps:playerScore, as:aiScore, run:false, point:true
          });
          lastTelemetrySample = pointT;
        }
      }
      ...
      if (playerScore >= 5 || aiScore >= 5) {
        ...
        draw();
        if (playerScore > aiScore) void submitVerifiedWin();
        return;
      }
      running = false;
      resetBall(!playerWon);
    ...
    const setPointerPaddle = e => {
      if (!active) return;
      const modelUV = window.VijvalHero3D?.screenUV(e);
      if (modelUV) {
        targetY = clampPaddle((1-modelUV.y)*H-paddleH/2,leftX);
        return;
      }
      if (window.VijvalHero3D) return;
    ...
    artboard.addEventListener('pointermove',e=>{
      if (window.VijvalHero3D) setPointerPaddle(e);
    },{passive:true});
    ...
    artboard.addEventListener('click', e => {
      ...
      const inScreen = window.VijvalHero3D
        ? window.VijvalHero3D.isScreenHit(e)
        : !!e.target.closest('.figma-computer-screen-wrap');
      e.preventDefault();
      e.stopImmediatePropagation();
    ...
    pong = Object.freeze({ enter, exit, isActive:()=>active });
    try {
      Object.defineProperty(window,'VijvalPong',{value:pong,writable:false,configurable:false,enumerable:false});
    } catch (_) {
