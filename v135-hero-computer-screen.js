(() => {
  ...
  if (!root.classList.contains('hero-intro-pending')) {
    ...
    // Primary trigger: workstation has started entering, so the screen should
    // already feel alive while the rest of the hero assembles.
    window.addEventListener('hero:computer-ready', begin, { once:true });

    // Compatibility fallback in case an older hero runtime is ever restored.
