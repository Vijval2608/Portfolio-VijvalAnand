(() => {
  ...
  const interactiveSelector = [
    ...
    '.figma-work-item',
    '.figma-work-media-shell',
    '[data-v276-interactive]',
    '.figma-work-view',
    '.viewer-close',
    '.viewer-backdrop',
    '.v204-about-trigger',
  ...
  const render = () => {
    ...
    const follow = interactive ? .36 : surface ? .28 : .24;
    fx += (x - fx) * follow;
    fy += (y - fy) * follow;

    const speed = Math.min(26, Math.hypot(velocityX,velocityY));
    const targetAngle = interactive ? 0 : Math.max(-10,Math.min(10,velocityX * .72));
    angle += (targetAngle - angle) * .22;

    const targetStretchX = 1 + Math.min(.14,speed * .0052);
    const targetStretchY = 1 - Math.min(.065,speed * .0024);
    stretchX += (targetStretchX - stretchX) * .22;
    stretchY += (targetStretchY - stretchY) * .22;
    ...
    const moving =
      ...
      Math.abs(y - fy) > .18 ||
      Math.abs(angle) > .04 ||
      Math.abs(stretchX - 1) > .002 ||
      Math.abs(stretchY - 1) > .002 ||
      Math.abs(velocityX) + Math.abs(velocityY) > .08;

    if (moving && !document.hidden) cursorRaf = requestAnimationFrame(render);
  };

  const requestCursorFrame = () => {
    if (!cursorRaf && !document.hidden) cursorRaf = requestAnimationFrame(render);
  };
  ...
  document.addEventListener('pointermove', event => {
    ...
    velocityX = nextX - prevPointerX;
    velocityY = nextY - prevPointerY;
    prevPointerX = nextX;
    prevPointerY = nextY;
    x = nextX;
    y = nextY;
  ...
  document.addEventListener('pointerover', event => { setMode(event.target); requestCursorFrame(); }, { passive:true });
