(() => {
  ...
  const updateNav = () => {
    navScrollRaf = 0;
    if (!nav) return;
    const next = window.scrollY > 16.2;
    if (next === navScrolledState) return;
    navScrolledState = next;
    nav.classList.toggle('is-scrolled', next);
  };
  const requestNavUpdate = () => {
    if (!navScrollRaf && !document.hidden) navScrollRaf = requestAnimationFrame(updateNav);
  };
  updateNav();
  ...
  const drawWave = () => {
    ...
    raf = 0;
    if (!heroVisible || document.hidden || !audio || audio.paused || !bars.length) return;
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      if (raf) cancelAnimationFrame(raf);
      raf = 0;
    } else if (heroVisible && audio && !audio.paused) {
      drawWave();
    }
  }, { passive: true });
  ...
  /* ----------------------------------------------------------------------
     V217 DESKTOP HERO INTRO — true geometric reverse of the approved Hero close

     The entrance deliberately uses the same scale / opacity / Z / convergence
     math as the scroll exit, only driven backward. No overshoot, no pop-out,
     no second animation language. The Macintosh establishes the scene first,
     slightly higher than its resting point, then the collapsed Hero planes
     smoothly resolve out of it while the computer settles down.
     ---------------------------------------------------------------------- */
  const desktopDepthIntro = window.matchMedia('(min-width: 769px)').matches;
  if (desktopDepthIntro) {
    /* V218: desktop entrance is owned entirely by v218-hero-motion.js.
       Do not run any separate GSAP entrance choreography here. Keeping
       hero-intro-pending intact lets the unified motion controller take the
       exact approved scroll-collapse state and replay it from 1 -> 0. */
    window.__heroDesktopBaseReady = true;
    window.dispatchEvent(new CustomEvent('hero:desktop-base-ready'));
    return;
  }
  ...
  // Opening hierarchy: Macintosh first, then one continuous wave through the name.
  springLetters(firstChars, .46);
  springLetters(lastChars, .76);
  ...
  // content enters.
  const computerBeat = .06;
  const workstationBeat = 1.52;
  const contentBeat = 2.20;

  // Beat 1 — after the Macintosh has established the scene, bring in identity metadata.
  if (role) {
    tl.to(role, {
      opacity: 1,
      x: 0,
  ...
  if (computer) {
    ...
    // Macintosh is the first visual beat. Start the CRT while the chassis is
    // still settling, then let the name arrive behind/around it.
    tl.call(() => {
      window.dispatchEvent(new CustomEvent('hero:computer-ready'));
    }, null, computerBeat + .13);
  }
