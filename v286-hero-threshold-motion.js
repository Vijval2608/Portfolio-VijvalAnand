/* V286 — 3D-first startup. The proven v266/v271 animation timeline is
   unchanged, but it is now allowed to start only after BOTH the desktop Hero
   base and the first real WebGL/Three.js frame are ready. No desktop image
   fallback participates in first paint. */
(() => {
  'use strict';
  ...
  addEventListener('hero:3d-runtime-failed',failOpen,{once:true});
  tryStart();

  /* This does NOT start the animation without 3D. It is only an escape hatch
     for a genuinely wedged request so the visitor is not trapped forever. */
  setTimeout(()=>{ if(!started && !modelReady) failOpen(); },30000);
})();
