...
## v297 — favicon + manual copy-case fix
- Added the approved retro monitor favicon as `favicon-v297.png` and linked it in `index.html` via `rel="icon"` and `rel="shortcut icon"`.
- Updated the first About paragraph opening from `i` to `I`.
- Updated the lowercase `there’s` sentence in the About section to `There’s`.
- No visual or interaction logic changes elsewhere; based on deployed v296.

## v298 — provided SVG logo integration
- Added the user-provided SVG as `logo-v298.svg`.
- Replaced the favicon links in `index.html` so the website uses the provided SVG as the tab icon.
- Replaced the navbar wordmark text `PORTFOLIO` with the provided SVG logo mark.
- Added a small CSS override to size the nav logo image cleanly on desktop and mobile.
- Preserved the earlier About copy capitalization fixes from v297.

## v299 — alternating retro favicon
- Added two matching favicon states that alternate every 1 second in browser tabs.
- State A preserves the approved striped orange V mark.
- State B keeps the exact same frame/silhouette but switches the V to a solid orange fill for a subtle retro signal/pulse effect.
- Added `favicon-cycle-v299.js` as a tiny external script so CSP inline hashes remain unchanged.
- Navbar logo remains the approved static SVG mark from v298.
- No other layout, 3D, interaction, or content behavior changed.

## v300 — higher-contrast alternating favicon states
- Removed the outer cream stroke from both alternating favicon states to make the swap read more clearly at tiny tab size.
- Kept the first alternating state as the striped orange `V` on a dark rounded-square background.
- Updated the second alternating state by swapping the two remaining colors, making it a dark `V` on an orange rounded-square background.
- Kept the 1-second favicon cycle script unchanged.


## v301 — restore PORTFOLIO navbar + quicker favicon cadence
- Restored the original navbar wordmark text `PORTFOLIO` and removed the v298 SVG navbar-logo CSS override.
- Kept the two high-contrast v300 favicon states unchanged.
- Reduced the alternating favicon interval from 1000 ms to 700 ms for a more active but still readable tab-bar rhythm.
- Preloads both favicon SVG states and warms them in the cycle controller to avoid cold-cache flicker between swaps.
- Switched the favicon controller to a chained timer and resets cleanly after background-tab throttling so it does not visibly catch up.
- No changes to Hero, 3D, Pong, Work, PDF viewer, or other portfolio interactions.

## v302 — final deployment cleanup
- Deployment-clean packaging pass based directly on v301; no visual, copy, interaction, animation, timing, or runtime behavior changes.
- Removed seven superseded/unreferenced leftovers that are not loaded by the active site: `favicon-v297.png`, `logo-v298.svg`, `hero-3d-loader-v288.js`, `section-spring.js`, `v286-hero-threshold-motion.js`, `v291-hero-threshold-motion.js`, and `figma-work.js`.
- Preserved all active v301 runtime files byte-for-byte, including the 700 ms alternating favicon behavior, current navbar `PORTFOLIO` wordmark, reliable 3D startup, current Hero/Works handoff, Fittribe viewer fix, Pong interactions, media optimizations, and security/deployment configuration.
- Regenerated `SHA256SUMS.txt` after cleanup.

## v303 — defer Pong hover hint until computer settles
- Based directly on the deployment-clean v302 package.
- The computer keeps its existing interactive cursor/hover state throughout boot, CRT power-on, BOOTING, and relocation.
- The `psst... wanna play pong?` cursor tooltip is now gated by the existing authoritative `hero:computer-settled` event, so it cannot appear while the computer is booting or translating.
- Added `hero:intro-complete` only as a restored/reduced-motion safety fallback; no new timing constants or Hero choreography changes were introduced.
- No changes to 3D, Pong gameplay, cursor behavior, favicon cadence, Work transitions, copy, layout, security headers, Netlify configuration, or server verification.

## v304 — active/inactive favicon cadence
- Kept the current two alternating favicon states unchanged.
- Replaced the favicon cycle script with `favicon-cycle-v304.js`.
- Active tab cadence is now slower at 1000 ms.
- Inactive/background tab cadence is now faster at 450 ms so the favicon blink is more attention-catching when the user is on another tab.
- Removed the old hidden-tab pause behavior; the favicon now continues cycling in the background.
