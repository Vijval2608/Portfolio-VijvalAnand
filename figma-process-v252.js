(() => {
  ...
  const processCards = [...(section?.querySelectorAll('.figma-film-card-process') || [])];
  const equalHeightCards = window.matchMedia('(max-width: 600px), (min-width: 601px) and (max-width: 1100px) and (orientation: portrait)');
  const portraitTablet = window.matchMedia('(min-width: 601px) and (max-width: 1100px) and (orientation: portrait)');
  ...
  let phoneCardHeightRaf = 0;

  const syncPhoneProcessCardHeights = () => {
    if (!processCards.length) return;

    section.style.removeProperty('--v249-process-card-height');

    if (!equalHeightCards.matches) return;

    if (phoneCardHeightRaf) cancelAnimationFrame(phoneCardHeightRaf);
    phoneCardHeightRaf = requestAnimationFrame(() => {
      phoneCardHeightRaf = 0;

      /* With the shared height variable removed, CSS lets each card size to
         its own readable content. Measure those natural boxes, then promote
         the tallest one to the shared height for the complete card set. */
      const tallest = Math.ceil(
        processCards.reduce((max, card) => {
      ...
      if (tallest > 0) {
        section.style.setProperty('--v249-process-card-height', `${tallest}px`);
      }
    });
  };
  ...
  const setup = () => {
    clearMotion();
    syncPhoneProcessCardHeights();

    const breakpoint = readNumber('--fp-desktop-breakpoint', 774);
    if (reducedMotion || portraitTablet.matches || window.innerWidth < breakpoint) return;
