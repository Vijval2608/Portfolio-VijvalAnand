(() => {
  ...
  // The computer remains fully interactive throughout boot/relocation, but the
  // Pong hint is only eligible once the approved Hero choreography reports
  // that the Macintosh has reached its normal resting position.
  let computerSettled =
    computer.dataset.v264BootPhase === 'settled' ||
  ...
  const syncVisibility = () => {
    const next = computerSettled && hovering && !pongActive() && !document.hidden;
  ...
  const unlockHint = () => {
    if (computerSettled) return;
    computerSettled = true;
    syncVisibility();
    requestFrame();
  };

  // `hero:computer-settled` is emitted at the exact end of the Macintosh
  // relocation. `hero:intro-complete` is retained only as a harmless safety
  // fallback for restored/reduced-motion states.
  addEventListener('hero:computer-settled', unlockHint, { once:true });
  addEventListener('hero:intro-complete', unlockHint, { once:true });
