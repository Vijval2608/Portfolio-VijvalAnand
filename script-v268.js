...
// Very soft cursor light on desktop. V267: render only while it is actually
// catching up with the pointer instead of burning a permanent 60fps loop.
...
if (cursorGlow && window.matchMedia('(pointer:fine)').matches && !prefersReducedMotion) {
  ...
  const animateGlow = () => {
    ...
    if (Math.abs(tx - gx) > .18 || Math.abs(ty - gy) > .18) {
      glowRaf = requestAnimationFrame(animateGlow);
    }
  };

  const requestGlow = () => {
    if (!glowRaf && !document.hidden) glowRaf = requestAnimationFrame(animateGlow);
  };
