(() => {
  ...
  function bindPreviewInteractions(p) {
    ...
    visual.addEventListener('pointermove', e => {
      ...
      if (art) art.style.transform = `perspective(900px) rotateX(${(-y*2.2).toFixed(2)}deg) rotateY(${(x*2.8).toFixed(2)}deg) scale(1.012)`;
  ...
  function renderPreview(animate=false){
    ...
    const replace = () => {
      ...
      if (animate && window.gsap && !reducedMotion) {
        ...
        gsap.fromTo(visual,{opacity:.5,scale:.988,y:8.1},{opacity:1,scale:1,y:0,duration:.52,ease:'back.out(1.12)',overwrite:true});
        gsap.fromTo(info,{opacity:0,y:7.2},{opacity:1,y:0,duration:.34,stagger:.03,ease:'power3.out',overwrite:true});
      }
    };

    if (animate && window.gsap && !reducedMotion && previewEl.children.length) {
      if (previewTween) previewTween.kill();
      previewTween = gsap.to(previewEl.children,{opacity:.2,y:-4.5,duration:.14,ease:'power2.in',stagger:.01,onComplete:replace,overwrite:true});
    } else replace();
  }
  ...
  function setActiveProject(id, scrollPreview=false){
    ...
    syncActiveList();
    renderPreview(true);
    if(scrollPreview && window.innerWidth<774.9) {
  ...
  moreBtn?.addEventListener('click', () => {
    ...
    if (window.gsap && !reducedMotion) {
      gsap.from(listEl.querySelectorAll('.work-item'),{opacity:0,y:5.4,duration:.34,stagger:.025,ease:'power2.out',overwrite:true});
    }
  });
  ...
  if(window.gsap && window.ScrollTrigger && !reducedMotion){
    ...
    mm.add('(min-width: 774.9px)', () => {
      ...
      stories.forEach((story, index) => {
        ...
        gsap.fromTo(copy,{opacity:.48,y:23.4},{opacity:1,y:0,ease:'none',scrollTrigger:{trigger:story,containerAnimation:horizontal,start:'left 86%',end:'left 55%',scrub:.75}});
        if(graphic) gsap.fromTo(graphic,{opacity:.62,scale:.965,y:12.6,rotate:index%2?1:-1},{opacity:1,scale:1,y:0,rotate:0,ease:'none',scrollTrigger:{trigger:story,containerAnimation:horizontal,start:'left 90%',end:'left 53%',scrub:.8}});
      });
      ...
      if (understand) {
        gsap.to('.signal-card.s1',{x:68.4,y:43.2,rotate:1,ease:'none',scrollTrigger:{trigger:understand,containerAnimation:horizontal,start:'left 82%',end:'center center',scrub:.8}});
        gsap.to('.signal-card.s2',{x:73.8,y:-46.8,rotate:-1,ease:'none',scrollTrigger:{trigger:understand,containerAnimation:horizontal,start:'left 82%',end:'center center',scrub:.8}});
        gsap.to('.signal-card.s3',{x:-68.4,y:30.6,rotate:0,ease:'none',scrollTrigger:{trigger:understand,containerAnimation:horizontal,start:'left 82%',end:'center center',scrub:.8}});
      ...
      if (structure) {
        ...
        gsap.to('.tiny-runner',{x:'29vw',y:-2.7,duration:2.4,repeat:-1,yoyo:true,ease:'sine.inOut'});
      }
      if (prototype) {
        gsap.to('.wireframe-window',{x:-10.8,y:6.3,rotate:-6,duration:2.6,repeat:-1,yoyo:true,ease:'sine.inOut'});
        gsap.to('.polished-window',{x:9.9,y:-5.4,rotate:5,duration:2.3,repeat:-1,yoyo:true,ease:'sine.inOut'});
        gsap.to('.cursor-arrow',{x:-26.1,y:-21.6,rotate:-7,duration:1.8,repeat:-1,yoyo:true,ease:'power2.inOut'});
      }
      if (ship) {
