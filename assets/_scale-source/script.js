...
if (window.gsap && window.ScrollTrigger && !prefersReducedMotion) {
  ...
  heroTl
    .from('.hero-eyebrow > span', { y: 10, opacity: 0, duration: .48, stagger: .06 }, .10)
    .from('.hero-title', { y: 30, opacity: 0, duration: .9 }, .12)
    .from('.hero-bottom', { y: 17, opacity: 0, duration: .68 }, .29)
    .from('.portrait-card', { y: 30, rotate: 3, scale: .975, opacity: 0, duration: .9 }, .23)
    .from('.floating-note', { y: 10, opacity: 0, scale: .97, duration: .55, stagger: .09 }, .58);
  ...
  revealItems.forEach(item => {
    ...
    item.style.transform = 'translateY(14px) scale(.985)';
