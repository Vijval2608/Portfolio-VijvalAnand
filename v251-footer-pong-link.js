...
(() => {
  ...
  const desktopPongLink = matchMedia('(min-width:775px)');
  const syncAvailability = () => {
    /* V277 — the footer shortcut is a deliberate navigation item, so keep it
       visible on layouts where the Hero computer exists instead of hiding it
       based on an off-screen render check. */
    const available = desktopPongLink.matches;
    link.classList.toggle('is-pong-available', available);
  ...
  desktopPongLink.addEventListener?.('change', syncAvailability);
})();
